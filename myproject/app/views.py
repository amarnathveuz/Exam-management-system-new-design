...
def exam_create(request):
    ...
    question_bank = Main_Question_Bank.objects.all()
    context = {
    ...
    if request.method == "POST":
        ...
        for section_title,section_count,modal_status in main_zip:    
            if modal_status != "direct_question" :
                ...
                section_save = Main_Exam_section.objects.create(Exam_id_id = exam_save.id ,section_title = section_title,
                        section_type = "Question",created_by = request.user
                        )
            #  Main_Question_Bank save    
            section_question_status = request.POST.getlist("section_question_status"+str(section_count))
            ...
            question_zip = zip(section_question_status,question,eng_question,ar_question,hi_question,ur_question,ta_question,question_description,question_comment,model_count)
            for  section_question_status,question,eng_question,ar_question,hi_question,ur_question,ta_question,question_description,question_comment,model_count in question_zip:
                if(section_question_status == "False"):
                    pass
                else:
                    ...
                    if modal_status == "direct_question":
                            ...
                            section_save.save()
                            Section_Question_Mapping.objects.create(Section_id_id = section_save.id,Exam_id_id =exam_save.id ,Question_id_id = question_save.id,created_by = request.user)
                    else:
                        Section_Question_Mapping.objects.create(Section_id_id = section_save.id,Exam_id_id =exam_save.id ,Question_id_id = question_save.id,created_by = request.user)
                        pass
...
def attend_user(request):
    exam_id = request.GET.get("exam_id")
    attend_user_details = Exam_attend_user.objects.filter(exam_id=exam_id)
    context = {
...
def see_result_admin(request):
    ...
    exam_id = request.GET.get("exam_id")
    data_main_exam = Main_Exam_Master.objects.get(id=exam_id)
    data_answer1 = exam_attend_user_score.objects.filter(exam_attend_user_id_id=user_id)
    if data_answer1:
        ...
        return render(request, 'see_result_admin.html',
    ...
    if question_remove_status == "0":
        ...
        if question_image :
            ...
            if extension == ".pdf" or extension == ".txt" or extension == ".doc" or extension == ".docx" :
                ...
                resized_image.save("media/Question_Bank_image/"+question_image.name)
                image_new1 = 'Question_Bank_image/'+question_image.name
            Main_Question_Bank.objects.filter(id=updated_id).update(
                    Imagefield = image_new1
            )
        else:
            print("empyty")
            Main_Question_Bank.objects.filter(id=updated_id).update(
...
def exam_details_delete(request):
    if request.method == "POST":
        field_id = request.POST.get("field_id")
        field = Main_Exam_Master.objects.get(id=field_id)
        field.delete()
        ...
        return render(request,"exam_details_delete.html",{'data':data})
    
def choice_option_delete(request):
    if request.method == "POST":
        field_id = request.POST.get("field_id")
        # ssss
        field = Question_Bank_multiple_choice.objects.get(id=field_id)
        field.delete()
        messages.success(request,"Question Choice Deleted Successfully..")
        return redirect(request.META['HTTP_REFERER'])
    else:
        id = request.GET.get("id")
